...
const ImageDisplay: React.FC<{ src: string | null; isLoading: boolean }> = ({ src, isLoading }) => (
...
const StoryText: React.FC<{ text: string; isLoading: boolean }> = ({ text, isLoading }) => (
...
const ActionPanel: React.FC<{ choices: string[], onPlayerAction: (action: string) => void, isLoading: boolean }> = ({ choices, onPlayerAction, isLoading }) => (
...
export const GameScreen: React.FC<GameScreenProps> = ({ gameState, onPlayerAction }) => {
  const { imageUrl, currentScene, choices, isLoading } = gameState;
